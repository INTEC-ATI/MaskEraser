###############################################################################
# This code base from https://github.com/naoto0804/pytorch-inpainting-with-partial-conv(MIT License)
###############################################################################
import torch
...
class InpaintingLoss(nn.Module):
    ...
    def forward(self, input, mask, output, gt):
        ...
        loss_dict['tv'] = torch.mean(torch.abs(output_comp[:, :, :, :-1] - output_comp[:, :, :, 1:])) + torch.mean(torch.abs(output_comp[:, :, :-1, :] - output_comp[:, :, 1:, :]))
...
class PCBActiv(nn.Module):
    def __init__(self, in_ch, out_ch, bn=True, sample='none-3', activ='relu', conv_bias=False):
        ...
        if sample == 'down-5':
            ...
            self.conv = partialconv2d.PartialConv2d(in_ch, out_ch, kernel_size=5, stride=2, padding=2, bias=conv_bias, multi_channel=True, return_mask=True)
        elif sample == 'down-7':
            # self.conv = PartialConv(in_ch, out_ch, 7, 2, 3, bias=conv_bias)
            self.conv = partialconv2d.PartialConv2d(in_ch, out_ch, kernel_size=7, stride=2, padding=3, bias=conv_bias, multi_channel=True, return_mask=True)
        elif sample == 'down-3':
            # self.conv = PartialConv(in_ch, out_ch, 3, 2, 1, bias=conv_bias)
            self.conv = partialconv2d.PartialConv2d(in_ch, out_ch, kernel_size=3, stride=2, padding=1, bias=conv_bias, multi_channel=True, return_mask=True)
        else:
            # self.conv = PartialConv(in_ch, out_ch, 3, 1, 1, bias=conv_bias)
            self.conv = partialconv2d.PartialConv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, bias=conv_bias, multi_channel=True, return_mask=True)
